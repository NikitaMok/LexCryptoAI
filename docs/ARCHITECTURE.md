...
| Автоматизация проверок (поле `check`) | Не начата |
...
Ближайшая задача — заполнить поле `check` у правил, которые проверяются
детерминированно, и связать их с фактами, извлекаемыми `app/parsing/extract.py`.
Это даёт сквозной проход «контракт на входе — перечень нарушений на выходе»
до подключения LLM.
