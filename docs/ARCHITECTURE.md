...
| Договор | Локальная модель плюс матрица из 32 правил | `app/rules`; вызов Ollama ещё не подключён |
| Кошелёк | Из текста — адрес, вовне — возраст, активность, USDT | `app/aml` |
| Контрагент | Из текста — ИНН и имя, вовне — реестр | не начато; ИНН уже извлекается |
| PDF | Три блока и оговорка о ручной перепроверке | `app/report/pdf.py` |

Оркестрация — FastAPI. LangGraph в зависимостях есть, в прогоне не участвует.
...
Пробелы обязаны быть объявлены: сейчас это ст. 7 № 115-ФЗ и Инструкция № 181-И.
...
| HTTP, страница, PDF | Готово |
| Скоринг адреса по открытым данным | Готово, по флажку; не ст. 35 |
| Ollama на разборе договора | Не подключена |
| Сверка контрагента по ИНН | Не начата; ИНН извлекается |
| Docker Compose | Каркас API |
