...
Поиск по эмбеддингам подключается тем же методом `search` и уточняет результат
там, где формулировки расходятся лексически. Векторы корпуса кэшируются в
`data/chroma/norm_vectors.bin` и пересобираются скриптом
`python -m scripts.embed_norms`, если изменился текст норм или имя модели.
Включается флагом `DENSE_SEARCH=true` либо ключом `--dense`.

Разрыв виден на запросе «возврат валюты в Россию»: лексический слой не связывает
его с репатриацией. Эмбеддинги выводят ст. 19 ч. 9 № 173-ФЗ. Слои дополняют
друг друга, а не заменяют. Лексическое ограничение закреплено тестом.
...
| Поиск нормы по эмбеддингам | Готово — кэш `data/chroma/norm_vectors.bin`, флаг `DENSE_SEARCH` / `--dense` |
...
| HITL и eval | Частично: ручная оценка в отчёте; `scripts/eval_fixtures.py` |
...
для правил, которые не сводятся к регулярным выражениям. Кэш векторов:
`python -m scripts.embed_norms`, затем `DENSE_SEARCH=true` или `--dense`.
