...
не уходит**: его читает локальная Ollama. Из текста модель достаёт номер
кошелька и реквизиты контрагента (наименование, ИНН). Уже эти идентификаторы —
не сам договор — запрашиваются во внешних источниках: публичные данные
блокчейна по адресу и открытые реестры по юрлицу. На выходе PDF с тремя
...
3. Локальная модель разбирает договор на соответствие № 282-ФЗ, № 283-ФЗ,
   115-ФЗ, 173-ФЗ и Инструкции Банка России № 181-И: квалификация
   внешнеторговой сделки, актив и сеть, адрес-идентификатор, курс,
   реквизиты сторон, пороговые оговорки.
4. Параллельно из того же текста извлекаются номер кошелька (TRON / EVM)
   и данные контрагента. Для скоринга адреса сервис обращается к публичным
   RPC (TronGrid, Etherscan). Для сверки юрлица — к открытым реестрам
   по ИНН и наименованию. В сеть уходят номер адреса, ИНН и имя стороны,
   не файл договора.
...
от 3 млн рублей — постановка на учёт в банке (Инструкция № 181-И).
...
Пока слой Ollama в коде не подключён, та же команда поднимает проверку
по матрице правил без модели. Оценка кошелька включается флажком на странице
и ходит в интернет. Сверка контрагента по ЕГРЮЛ ещё не сделана.
...
Сейчас работает разбор PDF/DOCX, матрица правил, страница в браузере
и PDF без языковой модели. Каталог источников
([`config/providers.yaml`](config/providers.yaml)) уже задаёт бесплатный
режим по умолчанию и слоты под платные API. Кошелёк по TronGrid/Etherscan
оценивается, если включить флажок. GoPlus, ЕГРЮЛ, Rusprofile, СБИС и Ollama
в прогон ещё не встроены: ИНН из текста извлекается, в реестр не уходит.
...
Автоматизировано 22 правила из 32, из 17 обязательных — 16. Образцовый
...
Дальше по коду: локальная Ollama на разборе договора, скоринг кошелька
в том же прогоне, сверка контрагента по ИНН, оговорка о ручной перепроверке
на первой странице PDF.
