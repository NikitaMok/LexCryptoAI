...
Интерфейс: `http://127.0.0.1:8000`. Контракт загружается в браузере, заключение
приходит JSON-ом и собирается в PDF. Файл после проверки удаляется.
...
Проверка контракта работает от начала до конца — пока без LLM, на матрице правил
и разборе текста:
...
На выходе цветовой статус, перечень нарушений с номерами пунктов договора
и ссылками на нормы, отдельный блок норм, вступающих в силу позднее, и список
правил, вынесенных на оценку юриста. С ключом `--quote-norms` под каждым
нарушением приводится дословный текст нормы из выверенного корпуса. Если ссылки
нет, а есть только формулировка, нормы ищутся лексически:
...
Открыть `http://127.0.0.1:8000`. Оценка адресов — по флажку, по умолчанию выключена:
это не цифровой анализ по статье 35 № 282-ФЗ.
...
Норма, текст которой не выверен по действующей редакции, помечается в заключении
как невыверенная. Пересказать её по памяти система не может — это ограничение
закреплено тестом.

Под нарушением приводится формулировка-исправление из матрицы правил. Сервис
не предлагает способов обойти требование: часть 2 статьи 30 № 282-ФЗ запрещает
предоставлять резидентам программное обеспечение для сделок с цифровой валютой
в нарушение закона. Заключение не выдаётся, если в него попал текст, который
можно прочитать как совет обойти норму.

Готово: матрица из 32 правил в восьми группах, 17 из них обязательные
([`docs/RULES_SPEC.md`](docs/RULES_SPEC.md)); разбор PDF и DOCX с приведением
текста к нумерованным пунктам; извлечение адресов, тикеров, сетей, сумм, ИНН
и маркеров квалификации сделки; автоматическая проверка 22 правил из 32,
включая пороговые; нормативный корпус из 264 норм с точным разрешением ссылок
вида «ст. 1 ч. 7 п. 1» в текст нормы; лексический поиск нормы по формулировке
